...
- [x] **Step 1: Full test suite + line limits**
...
- [x] **Step 2: Use superpowers:requesting-code-review, then superpowers:finishing-a-development-branch** to review and merge `feature/v2-rebuild` into `main`.
  Review verdict: ready with fixes. Applied before merge: in-place save before stash (unsaved-edit loss), ensureHostModules guard on rename relink, host-side name re-validation, thumbnail URL encoding, EvalScript-error toast fix, safeName parity test. Merged fast-forward at 8ef19ce, 41/41 tests green, branch deleted.
  Deferred minors (revisit in Phase 2): v1 localStorage migration can clobber newer library_path.txt; closeModal runs before acquire (typed category lost on busy bridge); usage stats increment before import result; failed stash leaves orphan folder; Enter doesn't confirm delete modal (arguably safer); relink filename map should skip comp.png/metadata.json; split actions.js per its TODO marker.
