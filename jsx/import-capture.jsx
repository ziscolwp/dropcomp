...
// saveVerifiedThumb, aeProjectFilesIn, aeProjectExt, ensureHostModules.
// Uses aep-compat globals: aepPreflight, aepImportFailureMessage.
...
function captureCompInfo(aepPath, targetPngPath, preferredName) {
    ...
    try {
        ...
        if (ensureHostModules()) {
            ...
            if (!pf.ok) return { ok: false, error: pf.message, incompatible: true };
        }
        ...
        var msg = ensureHostModules() ? aepImportFailureMessage(aepPath, e.toString()) : e.toString();
...
function addExternalAep(libraryPath, categoryName, sourceAepPath) {
    try {
        ...
        // preflight BEFORE copying so an incompatible file leaves no trace
        if (ensureHostModules()) {
            var pf = aepPreflight(src.fsName);
            if (!pf.ok) return jerr(pf.message);
        }
